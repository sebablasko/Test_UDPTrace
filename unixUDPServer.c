...
void print_usage(){
    printf("Uso: ./internetUDPServer [--verbose] --packets <num> --threads <num> [--scheduler <SchedSchemmaName> --setcpu <num>]\n");
}
...
void parseArgs(int argc, char **argv){
	...
	while (1){
		...
		static struct option long_options[] = {
         ...
         c = getopt_long (argc, argv, "vd:t:s:c:",
...
void llamadaHilo(int socket_fd){
  ...
  //Marca FTRACE
  if(enabledTrace) write(marker_fd, "MITRACE UNIX: Nuevo Thread\n", 27);

	for(i = 0; i < paquetesParaAtender; i++) {
    if(enabledTrace) write(marker_fd, "MITRACE UNIX: Comienza el read del socket\n", 42);
...
int main(int argc, char **argv){
  ...
  int pid = getpid();
	...
	int socket_fd;
	socket_fd = unix_udp_bind(MY_SOCK_PATH);
	if(socket_fd < 0) {
		fprintf(stderr, "Error al crear/tomar el socket Unix\n");
		exit(1);
	}
